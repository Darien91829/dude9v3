...
async function fetchAndRenderChronologicalList(filterTerm = "") {
  ...
  // Get current week window boundary targets in Unix time seconds
  ...
  try {
    ...
    // Map items out natively grouped by local conversion days
    const mappedDays = {};
...
async function fetchConsumetStreamWithStack(animeTitle, epNum, dubMode) {
  ...
  for (const provider of CONSUMET_STACK) {
    try {
      ...
      // Step 1: Search provider endpoint to map out internal string identifiers
      ...
      // Step 2: Grab full internal episode metadata tree tracking lists
      ...
      // Step 3: Call extraction watch route to grab target streaming URL arrays
...
async function launchVideoPlayer(epNum) {
  ...
  if (activeSourceMode === 'mal') {
    ...
    if (backupStreamUrl) {
      if(noticeOverlay) noticeOverlay.classList.add('hidden');
      iframe.classList.remove('hidden');
      // If returning an m3u8 playlist chunk directly, standard browser iframes need an HLS wrapper context proxy player
...
function updateServerButtonsUI() {
  ...
  ['mal', 'cdn-9anime', 'consumet'].forEach(src => {
